$(function(){
    var data = {"list":[
        {time:1592285738000,warningLevel:2.8,level:2.05,predictLevel:2.50,tideLevel:2.10,waterLevel:2.00},
        {time:1592372138000,warningLevel:2.9,level:2.50,predictLevel:3.10,tideLevel:2.30,waterLevel:2.40},
        {time:1592458538000,warningLevel:2.88,level:2.55,predictLevel:3.20,tideLevel:2.45,waterLevel:3.20},
        {time:1592544938000,warningLevel:2.8,level:2.05,predictLevel:2.50,tideLevel:2.10,waterLevel:2.00},
        {time:1592631338000,warningLevel:2.9,level:2.50,predictLevel:3.10,tideLevel:2.30,waterLevel:2.40},
        {time:1592717738000,warningLevel:2.88,level:2.55,predictLevel:3.20,tideLevel:2.45,waterLevel:3.20},
        {time:1592804138000,warningLevel:2.8,level:2.05,predictLevel:2.50,tideLevel:2.10,waterLevel:2.00},
        {time:1592890538000,warningLevel:2.9,level:2.50,predictLevel:3.10,tideLevel:2.30,waterLevel:2.40},
        {time:1592976938000,warningLevel:2.88,level:2.55,predictLevel:3.20,tideLevel:2.45,waterLevel:3.20}
    ]};
    /*data.list = [{
    ...
    var myEchart = echarts.init(document.getElementById('procedureChart'));
    var xAxisData = [];
    var seriesOneData = [];
    var seriesTwoData = [];;
    var seriesThreeData = [];
    ...
    //forecastReportStorm(113.62265,22.78826);
});

//水情专题/预报播放/风暴潮
function forecastReportStorm(lonPoint,latPoint){
    $("#forecastStormPop").show();
    //预报播放风暴潮的过程图和报表导航切换
    $(".forecastStorm-title-txt").click(function(){
        var index=$(this).index();
        $(this).addClass("forecastStorm-title-txtClick");
        $(this).siblings(".forecastStorm-title-txt").removeClass("forecastStorm-title-txtClick");
    ...
    $(".forecastStorm-title-txt").eq(0).trigger("click");
    var params = {
        'lonPoint': lonPoint,
        'latPoint': latPoint
    }
    $ajax('/web/opendap/netcdf/storm/water/regime/forecast/chart', params, function (res) {
        var data=res.data;
        console.log(data);
        if(data.length == 0){
            ...
            $(".forecastStormTableBody").html(tableHtml);
        }
        $("#forecastStormLonLat").text(lonPoint+' ；'+latPoint);
    });
}
//风暴潮统计图表
function stormChart(data){
    var myEchart = echarts.init(document.getElementById('procedureChartStorm'));
    var xAxisData = [];
    var seriesTwoData = [];;
    var seriesThreeData = [];
    var seriesFourData = [];
