 ...
 * @param {站点雨情数据} data 
 */
function loadRainfall(data) {
    var canvas = document.getElementById('canvasMapRainfall');
    canvas.width = 2000;
    canvas.height = 2000;
    // console.log(data);
    var n = data.length;
    ...
    var minData = parseFloat(data[0].rainLevel);
    var maxData = parseFloat(data[0].rainLevel);
    var lonMin = 109.65079001700008, lonMax = 117.31371173, latMin = 20.21417992, latMax = 25.51961632600006;
    for (var i = 0; i < n; i++) {
        if(data[i].rainLevel != 'null' && data[i].lng != 'null' && data[i].lat != 'null') {
            t.push(parseFloat(data[i].rainLevel)); // 权重值
            x.push(parseFloat(data[i].lng)); // x
            y.push(parseFloat(data[i].lat)); // y
            if(minData > parseFloat(data[i].rainLevel)) {
                minData = parseFloat(data[i].rainLevel)
            }
            if(maxData < parseFloat(data[i].rainLevel)) {
                maxData = parseFloat(data[i].rainLevel)
            }
        }
    }
    railfallLegendNum(minData, maxData);
    var variogram = kriging.train(t, x, y, "exponential", 0, 100);
    // console.log(variogram);
    
    // $.getJSON('./static/GeoJson/GuangdongProvince.json', function(resdata){
    //     console.log(resdata.features);
    //     var boundData = [];
    //     for(var i = 0; i < resdata.features.length; i++){
    //         var featuresData = resdata.features[i];
    //         var coordinates = featuresData.geometry.coordinates;
    //         for(var j = 0; j < coordinates.length; j++) {
    //             // console.log(coordinates[j]);
    //             var boundData0 = coordinates[j][0];
    //             boundData.push(coordinates[j][0]);
    //             // console.log(boundData0);
    //             for(var k = 0; k < boundData0.length; k++) {
    //                 // console.log(boundData0[k]);
    //                 if(lonMin > parseFloat(boundData0[k][0])) {
    //                     lonMin = parseFloat(boundData0[k][0])
    //                 }
    //                 if(latMin > parseFloat(boundData0[k][1])) {
    //                     latMin = parseFloat(boundData0[k][1])
    //                 }
    //                 if(lonMax < parseFloat(boundData0[k][0])) {
    //                     lonMax = parseFloat(boundData0[k][0])
    //                 }
    //                 if(latMax < parseFloat(boundData0[k][1])) {
    //                     latMax = parseFloat(boundData0[k][1])
    //                 }
    //             }
    //         }
    //     }
    ...
    kriging.plot(canvas, grid, [lonMin, lonMax], [latMin, latMax], colors);
    var temptureLayer = viewer.imageryLayers.addImageryProvider(new Cesium.SingleTileImageryProvider({
        url: canvas.toDataURL('image/png'),
        rectangle: new Cesium.Rectangle(
            Cesium.Math.toRadians(parseFloat(lonMin)),
            Cesium.Math.toRadians(parseFloat(latMin)),
            Cesium.Math.toRadians(parseFloat(lonMax)),
            Cesium.Math.toRadians(parseFloat(latMax))
          )
    }));
    temptureLayer.alpha = 0.5;
    temptureLayer.id = 'Rainfall';
}
...
function removeRainfallLayer() {
    for(var i = 0; i < scene.imageryLayers.length; i++) {
        var imagery = scene.imageryLayers.get(i);
        if(imagery.id && imagery.id == 'Rainfall') {
            scene.imageryLayers.remove(imagery)
            i--;
        }
    }
}